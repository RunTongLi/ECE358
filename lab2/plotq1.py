...
def graphEfficiency():
    data = readData("efficiency_1.csv")
    plt.title("Efficiency in persistent CSMA/CD Protocol")
    ...
    plt.savefig("efficiency_1.png", bbox_inches="tight")
    plt.close()

def graphThroughput():
    data = readData("throughput_1.csv")
    plt.title("Throughput in persistent CSMA/CD Protocol")
    x = [float(point) for point in data[0]]
    a7 = [float(point) for point in data[1]]
    a10 = [float(point) for point in data[2]]
    a20 = [float(point) for point in data[3]]
    ...
    plt.savefig("throughput_1.png", bbox_inches="tight")
    plt.close()
